...
class com.mbudm.mbFormModule extends MovieClip{
	...
	public function onSubmit(mc:MovieClip){
		...
		if(allClear){
			...
			var arr = pageXMLobj.form.attributes.action.split(".")
			var ext = arr[arr.length - 1].toLowerCase();
			
			if(ext == "txt"){
				_root.tracer.htmlText+=ext+" is txt file so using load method";
				result_lv.load(pageXMLobj.form.attributes.action);
			}else{
				_root.tracer.htmlText+=ext+" is not a txt file so using sendAndLoad method";
