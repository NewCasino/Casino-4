...
class com.mbudm.mbScroller extends MovieClip{
	...
	private var targetScrollPoint:Number;
	...
	public function scrollTo(p:Number){
		tweening = true;
		var dir = p > paneContent[scrollPosProp] ? true : false;
		tweenTo = dir ? Math.min(basePos,p) : Math.max(minPos,p);
		tweenCount = 20;
		
		scrollingMode = 7; // flag that this is an external trigger for the tween, currently this will only be important if the previous scroll was a slider drag - changing to 7 will make the updateSliderPosition work
		startInterval();
	...
	private function setScrollPoint(scrollPos:Number){
		...
		updateSliderPosition();
		//broadcaster.broadcastMessage("onScrollingUpdate");
	}
