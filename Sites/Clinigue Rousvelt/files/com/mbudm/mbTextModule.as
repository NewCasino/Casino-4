...
class com.mbudm.mbTextModule extends MovieClip{
	...
	private var page_chapters:Array
	...
	public function init(pX:XML,ind:mbIndex,iC:String,t:mbTheme) {
		...
		txtBox_mc.addListener(this);
		
		if(toc_enabled){
			txtBox_mc.init(index,theme,"chapters");
			createTOC();
		}else{
	...
	private function createTOC(){
		...
		toc.init(pageContent,pageXMLobj.toc.attributes.tag);	
	...
	private function onTOCListReady(){
		toc_list = toc.getTOC();
		createTOCmenu();
		page_chapters = toc.getChapters();
		txtBox_mc.chapters_htxt = page_chapters;
