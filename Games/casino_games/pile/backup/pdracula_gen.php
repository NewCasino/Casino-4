...
//$adminemail="webmaster@xlnet.ltd.uk";
...
//Slot
//if( (isset($_POST["slot"]) && (($_POST["slot"])!="undefined") ){ $slot=$_POST["slot"]; }else{ $slot=1; }
if (isset($_POST["slot"]) ){ $slot=$_POST["slot"]; }else{ $slot=1; }
...
do{
	...
	$hd=99;
	switch($hand){
		case "Royal Flush":     $win=$total_bet*250; $hd=0; break;
		case "Strait Flush":    $win=$total_bet*50;  $hd=1; break;
		case "Four of a kind":  $win=$total_bet*25;  $hd=2; break;
		case "Full House":      $win=$total_bet*9;   $hd=3; break;
		case "Flush":           $win=$total_bet*6;   $hd=4; break;
...
//
//	if ($diff>0){$win1=$diff;}else{$win1=0;}
	// send to client new cards, transaction id, game result
	echo "&ans=OK2&ti=".$trans_id."&cd=".$cards."&w=".$win."&h=".$hd."&ub=".$user_balance;
}
