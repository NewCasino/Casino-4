...
if ( isset($_POST["tp"]) ){ $game_type=$_POST["tp"]; }else{$game_type=44; }
if( isset($_POST["uid"]) ){ $sid=$_POST["uid"]; }else{ exit; }
$result=mysql_query("select `userid` from `session` where `sid`='$sid'");
$userid=mysql_result($result, 0, "userid");
//if ( isset($_POST["mn"]) ){ $magic_number=$_POST["mn"]; }else{$magic_number=0; }
...
do{
	...
	$hd=99;
	switch($hand){
		case "5 of a Kind":     $win=$total_bet*200; $hd=0; break;
		case "Royal Flush":     $win=$total_bet*100; $hd=1; break;
		case "Strait Flush":    $win=$total_bet*50;  $hd=2; break;
		case "Four of a kind":  $win=$total_bet*20;  $hd=3; break;
		case "Full House":      $win=$total_bet*15;  $hd=4; break;
		case "Flush":           $win=$total_bet*10;  $hd=5; break;
		case "Strait":          $win=$total_bet*6;   $hd=6; break;
		case "Three of a kind": $win=$total_bet*3;   $hd=7; break;
		case "Two Pair":        $win=$total_bet*2;   $hd=8; break;
		case "Jacks or Better": $win=$total_bet*1;   $hd=9; break;
	}
$magic_number="c_".$_POST["mn"];
if(stristr($cards, $magic_number) != FALSE) { 
        $win=$win*2; 
   }
	$diff=$win-$total_bet;
...
elseif($stage==2){ // user hold some cards and wait for new ones
	...
	$gameid=mysql_result($result, 0, "gameid");
	$maxwin=mysql_result($result, 0, "comments");	$result=mysql_query("select `bet` from `games` where `gameid`='$gameid'");
...
// Magic Number
//	if ($x = array_search("c_8", $new_cards)) {$win=$win*2;}
//	$f=file_put_contents("log.txt", $cards); // (PHP 5) 

$magic_number="c_".$_POST["mn"];
if(stristr($cards, $magic_number) != FALSE) { 
        $win=$win*2; 
   } 
//$f=file_put_contents("log.txt", $magic_number); // (PHP 5)
	$diff=$win-$total_bet;
...
//	$co_amount = -$co_amount;
	$tm=time();
	if ($co_amount > 0) { 	mysql_query("insert into `transactions` values('', '0', '0', '$tm', 'co', '$co_amount', '$userid')"); }
	}
...
elseif($stage==3){ // user press `collect` and get his win
	...
	echo "&ans=OK3&ub=".$user_balance."&w=".$win;
}
...
do{
	...
	if($num1>$num2){ $res=0; $win=0; if ($_POST["betHalf"] == "true"){ $win=$_POST["w"]/2; } }       // player lost
	elseif($num1==$num2){ $res=1; }  // draw game
	else{ $res=2; $win*=2; if ($_POST["betHalf"] == "true"){ $win=$_POST["w"]*1.5; } }                  // player win
	$diff=$win-$total_bet;
