...
//Get Jackpot Amount
$result=mysql_query("select `amount` from `jackpot` where `type`='43'");
...
//}while($hd<7); // Deal new cards. Initial deal can not be better than "Ten or Better
	...
	//Log JP winning
	if ($win==$jackpot){
		mysql_query("update `jackpot` set `amount`=`amount`-$jackpot where `type`='43'");
	}

	//Update Jackpot 1/10 of the bet
	mysql_query("update `jackpot` set `amount`=`amount`+$total_bet/10 where `type`='43'");
