...
//Get Jackpot Amount
$result=mysql_query("select `amount` from `jackpot` where `type`='44'");
...
do{
	...
	$hd=99;
	switch($hand){
		case "Royal Flush":     $win=$jackpot; $hd=0; break;
...
//}while($hd<7); // Deal new cards. Initial deal can not be better than "Jacks or Better
	...
	//Log JP winning
	if ($win==$jackpot){
		mysql_query("update `jackpot` set `amount`=`amount`-$jackpot where `type`='44'");
	}

	//Update Jackpot 1/10 of the bet
	mysql_query("update `jackpot` set `amount`=`amount`+$total_bet/10 where `type`='44'");

	mysql_query("update `games` set `bet`='$cards' where `gameid`='$gameid'");

	// send to client transaction id, 5 card, new user balance
	echo "&ans=OK1&ti=".$trans_id."&cd=".$cards."&ub=".$user_balance."&jp=".$jackpot;
}
