...
function calWinLimit(){
	$result=mysql_query("select * from `settings` where `type`='52'");
...
//Get Jackpot Amount
//TODO Define (Jackpot)
$result=mysql_query("select `amount` from `jackpot` where `type`='52'");
...
//		mysql_query("update `jackpot` set `amount`=`amount`-$jackpot where `type`='52'");
		}
	}
	$maxwin=calWinLimit();
}while($win>$maxwin);
...
$tm=time();
mysql_query("insert into `games` values('', '$userid', '$tm', '$bet', '-1', '$rg', '$diff', '52')");
$gameid=mysql_insert_id();
mysql_query("insert into `transactions` values('', '$userid', '$gameid', '$tm', '$rg', '$diff', '$win_limit')");

//Update Jackpot 1/10 of the bet
mysql_query("update `jackpot` set `amount`=`amount`+$bet/10 where `type`='52'");
