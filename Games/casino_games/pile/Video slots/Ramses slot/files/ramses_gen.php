...
<?PHP
	...
	function countResult() {
		...
		global $b_game, $nLines, $symbolSet, $payLines, $pyramidBonus, $kingBonus, $bet;
		...
		//win multiplied by bet per line;
	...
	*/
	...
	//actually we don't need this var 
	$c_val = 1;
	...
	*/
		...
		//as I could understand global variables must appear here in main body, before using them in functions
		//so here is set for all vars we use later
		$symbolSet = array();
		$payLines = array()
		$kingBonus = false;
		$pyramidBonus = false;
		...
		//function generates uniqe simbols set with win < maxwin
		generateSymbolSet();
		
		//win result already multiplied by bet per line
		$win = countResult();
		
		$total_win = $win;
		
		//if there is a win generate numbers for gamble
		if ($win > 0) {
			$gambleNums = makeRandomArray(5, 1, 1);
			$gambleStr = arrayToString($gambleNums);		
		}
		
		//if there is king bonus make numbers for it
		if ($kingBonus) {
			$maxNumber = $total_bet;
			$kingWin = 0;
			do {
